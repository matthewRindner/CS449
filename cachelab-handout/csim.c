...
{
    //fills 2-d array
    cache = (cache_block**)calloc(sizeof(cache_block*), S);

    for(int i = 0; i < S; i++)
    {
        cache[i] = (cache_block*)calloc(sizeof(cache_block), E);
    }
...
void accessData(mem_addr_t addr)
{
    ...
    if(cache_set[eviction_index].used)
    {
            eviction_count++;
    }

    cache_set[eviction_index].used = 1;
    cache_set[eviction_index].tag = tag_bits;
    cache_set[eviction_index].lru = lru_counter++;
