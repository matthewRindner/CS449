...
void* add_to_freeList(Block* elem)
{
  elem->freeNode.prevFree = free_list_head;
  free_list_head = elem;
}

void* remove_from_freeList(Block* elem)
{
  Block* leftNeighbor = elem->freeNode.prevFree;
  Block* rightNeighbor = elem->freeNode.nextFree;
  rightNeighbor->freeNode.prevFree = leftNeighbor;
}
...
void* mm_malloc(size_t size) {
  ...
  Block* freeBlockFound = searchList(reqSize);

  if(freeBlockFound == NULL)
  {
    Block* new_block = requestMoreSpace(reqSize + sizeof(BlockInfo));
    //check that its prev pointer (new_block->info.prev) pints to end of the list
    new_block->info.size = reqSize;
    new_block->info.prev = malloc_list_tail;
    malloc_list_tail = new_block;
    ptrFreeBlock = new_block;
  }
  else
  {
      if((-1 *(freeBlockFound->info.size) - reqSize) >  sizeof(Block))
      {
        //somthing about splitBlock
        Block* split_block = (Block*)UNSCALED_POINTER_ADD(freeBlockFound, sizeof(BlockInfo)+reqSize);
        split_block->info.size = -1*(-1 *(freeBlockFound->info.size) - reqSize);
        split_block->info.size += sizeof(BlockInfo);
        split_block->info.prev = freeBlockFound;
        
        freeBlockFound->info.size = reqSize;
        ptrFreeBlock = freeBlockFound;


        if(next_block(split_block) != NULL)
        {

          next_block(split_block)->info.prev = split_block;
      ...
      {
        //allocate block
        freeBlockFound->info.size =  -1 * freeBlockFound->info.size;
        ptrFreeBlock = freeBlockFound;
      }
  }
...
void coalesce(Block* currBlock) {
  Block* nextBlock = next_block(currBlock);
  Block* previousBlock = currBlock->info.prev;
  Block* tmpBlock = NULL;

  // YOUR CODE HERE!
  if(nextBlock != NULL && (nextBlock->info.size < 0))
  {
    currBlock->info.size += nextBlock->info.size - sizeof(BlockInfo);

    if(malloc_list_tail == nextBlock)
    {
      malloc_list_tail = currBlock;
    }

    if(next_block(nextBlock) != NULL)
    {
      next_block(nextBlock)->info.prev = currBlock; 
    }
  ...
  {
    previousBlock->info.size += currBlock->info.size - sizeof(BlockInfo);

    if(malloc_list_tail == currBlock)
    {
      malloc_list_tail = previousBlock;
    }

    if(next_block(currBlock) != NULL)
    {
      next_block(currBlock)->info.prev = previousBlock; 
    }
...
void mm_free(void* ptr) {
  Block* currBlock = (Block*)UNSCALED_POINTER_SUB(ptr, sizeof(BlockInfo));

  //
  // YOUR CODE HERE!
  //--set size to negative and frelisthead to block info
  
  currBlock->info.size = -1*currBlock->info.size;
  //currBlock = free_list_head()
