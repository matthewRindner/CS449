...
int intSize() {
  ...
  intPtr1 = intArray;		//stores addr of beginning of array[0]
  intPtr2 = intArray + 1;	//stores addr of the second element in array, array[1]
  //the difference btwn the two addr will yeild the size of one element(int) in mem
...
int doubleSize() {
  ...
  doubPtr1 = doubArray;		//points to beginning of array
  doubPtr2 = doubArray + 1;	//points to array[1] of array
...
int pointerSize() {
  ...
  return (int)ptrPtr2 - (int)ptrPtr1; //casts to ints and reuturns differnce
}
...
void swapInts(int *ptr1, int *ptr2) {
  // Your code here
	int temp = *ptr1;	//stores value of ptr1
	*ptr1 = *ptr2;		//*ptr stores value of *ptr2
	*ptr2 = temp;		//*ptr2 stores value of temp
...
void stringAppend(char * d, char * s) {
  // Your code here
	//assgin pointer to start + length of string
	char* string_ptr = stringLength(d) + d;
	//traverse first string till null
	while(*s != '\0')
		//concatinate the remaining chars in second string till end of second string
		*string_ptr++ = *s++; 
	//add terminating char to concatinated string
	string_ptr = '/0';
...
void selectionSort(int arr[], int arrLength) {
	...
	// iterate through unsorted sarray  
